...
以下是使用合成数据（make_moons）进行的完整消融实验结果：

#### 1.1.1 三种对齐方法详细结果

**NCFM-amplitude（幅度对齐）结果：**
- **幅度变换场景**：
  - MSE: 0.026015
  - MMD: 0.000019
  - NCFM-幅度: **0.000031**
...
| 场景 | 对比 | 差异(%) | 结论 |
|------|------|---------|------|
| **幅度变换** | 幅度→复数 | **+122.9%** | 复数对齐更差（但损失值仍很小） |
| **相位变换** | 相位→复数 | **-89.4%** | **复数对齐更好（显著提升）** |
| **完全不同** | 相位→复数 | **-81.2%** | **复数对齐更好（显著提升）** |

**关键发现：**
- 在**相位变换**和**完全不同**场景下，复数对齐相比相位对齐有**显著优势**（降低 81-89%）
- 在**幅度变换**场景下，幅度对齐略优于复数对齐，但两者损失值都很小（0.000031 vs 0.000069）

### 1.2 CIFAR-10 数据集结果对比

下表展示了在 CIFAR-10 数据集上的三种对齐方法对比：
...
**合成数据结果：**
- **幅度对齐**：NCFM-幅度损失 = 0.000031（最低）
- **复数对齐**：NCFM-复数损失 = 0.000069（略高，但损失值仍很小）
- **量化差异**：+122.9%（复数对齐更差，但两者都在极低水平）
...
**结论**：在幅度变换场景下，幅度对齐和复数对齐都表现良好，损失值都很小。

#### 场景 2：相位变换（Phase Altered）

**合成数据结果：**
- **相位对齐**：NCFM-相位损失 = 0.930510
- **复数对齐**：NCFM-复数损失 = **0.098262**（最低）
- **量化差异**：**-89.4%**（复数对齐显著更好）
...
**结论**：复数对齐在相位变换场景下显著优于单一对齐方法，在两个数据集上都表现出色。

#### 场景 3：完全不同（Completely Different）

**合成数据结果：**
- **相位对齐**：NCFM-相位损失 = 1.925033
- **复数对齐**：NCFM-复数损失 = **0.361486**（最低）
- **量化差异**：**-81.2%**（复数对齐显著更好）
...
**结论**：复数对齐在完全不同的分布场景下仍能保持较低损失，在两个数据集上都表现最优。
...
1. **复数对齐（NCFM-both）综合表现最优**
   - 在**相位变换**和**完全不同**场景下，复数对齐显著优于单一对齐方法
   - **合成数据**：相位变换场景降低 89.4%，完全不同场景降低 81.2%
   - **CIFAR-10 数据**：相位变换场景降低 99.87%，完全不同场景降低 99.89%
   - 在**幅度变换**场景下，幅度对齐和复数对齐都表现良好（损失值都很小）

2. **量化差异显著**
   - 相位变换场景：复数对齐相比相位对齐降低 **81-99%**
   - 完全不同场景：复数对齐相比相位对齐降低 **81-99%**
   - 这些量化差异证明了复数对齐方法的有效性
...
**实验时间**：2025-11-23 10:08:05  
**数据集**：合成数据（make_moons）  
**实验配置**：`--align=all`（完整消融实验）
...
1. **幅度变换场景**：
   - 幅度对齐：0.000031
   - 复数对齐：0.000069
   - 虽然复数对齐损失值略高（+122.9%），但两者都在极低水平（< 0.0001）
...
- **负值（-）**：表示复数对齐更好（损失值更低）
- **正值（+）**：表示复数对齐更差（损失值更高）
- **在相位变换和完全不同场景下，复数对齐的优势非常明显（降低 81-89%）**

### 4.3 实验输出文件

运行完整消融实验后，会生成以下文件：
- `quantitative_differences_*.txt`：量化差异分析报告（包含 ±% 差异）
